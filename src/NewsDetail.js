...
const NewsDetail = () => {
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      />
      <Text style={styles.textLocation}>Europe</Text>
      ...
      <Text style={styles.textContent}>
        ...
        which Russia stands to make up to £250bn ($326bn) this year. There has
        been a growing frustration among Ukraine's leadership with Berlin, which
        has backed some sanctions against Russia but so far resisted calls to
        back tougher action on oil sales.
      </Text>

      <Text style ={{color: '#000000', fontWeight:'bold', marginTop: 20}}>Đọc thêm</Text>
      {
        dataJson.map((item) => <ItemlistNews key={item._id} data ={item}/>)
      }
    </ScrollView>
  );
};
